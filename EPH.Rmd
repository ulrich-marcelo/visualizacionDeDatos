...
Primero armo un violinplot para el ejercicio anterior
...
Acá vemos mucho mejor cada distribución salarial. En particular me llama la atención las "pendientes" más pronunciadas de las distribuciones de varones para los deciles más bajos. Es como si no hubiese casi varones ahí.

Se me ocurre que puede ser que no sean asalariados, que figuren como cuenta propista. O quizás son esos que filtramos por no tener la variable P21 en condiciones (salario menor a cero)...




Todo esto era para los asalariados, pero ahora quiero explorar el resto de la sociedad, en particular, a los inactivos. En particular: quienes son?


```{r}
data_inactivos <- calculate_tabulates(filter(data, ESTADO == "3"), x = "CAT_INAC",add.percentage = "col")
data_inactivos
```
...
ggplot(data_inactivos, aes(fill=CAT_INAC, area=Freq, label = scales::percent(Freq/100)))+
  geom_treemap()+
  geom_treemap_text(place = "center",)+
  theme_minimal()+
  labs(title="Composición de población inactiva",
       fill = "Categoría de Inactividad",
       caption = "Fuente: Encuesta Permanente de Hogares (EPH-INDEC), vía paquete de R 'eph' (Kozlowski et al, 2020)") +
  theme(
...
Estos estudiantes inactivos, son todos alumnos menores o son adultos?

```{r}
ggplot(filter(data, CAT_INAC == "3"), aes(x=CH06))+
  geom_density()+
  theme_minimal()+
  labs(title = "Distribución etaria de inactivos-estudiantes",
       x = "Edad",
       y = "densidad",
       caption = "Fuente: Encuesta Permanente de Hogares (EPH-INDEC), vía paquete de R 'eph' (Kozlowski et al, 2020)"
       )
...
Y la distribución etarea de estudiantes mayores de edad?

```{r}
ggplot(filter(data, CAT_INAC == "3" & as.numeric(CH06) > 18), aes(y=as.numeric(CH06), x="Mayores de edad"))+
  geom_violin()+
  theme_minimal()+
  labs(title="Distribución etaria para estudiantes mayores de 18 años",
       subtitle = "Categorizados como Inactivos ocupacionalmente por la EPH.",
       y = "Edad",
       caption = "Fuente: Encuesta Permanente de Hogares (EPH-INDEC), vía paquete de R 'eph' (Kozlowski et al, 2020)")
```

Efectivamente, lo esperable: los estudiantes mayores de 18 que no tienen una ocupación laboral suelen estar alrededor de los 20 años.


Muy poco analisis, lo sé. No estoy contento con esta entrega la verdad. Se me pasó y a esto es lo que llegué... Perdón profe </3
